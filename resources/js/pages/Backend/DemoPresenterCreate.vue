...
const breadcrumbItems = [
    { title: "Home", href: "/" },
    { title: "Demo Presenter Entry", href: "/presenterCreate" },
];
...
const updateStatus = async (id: number, newStatus: string) => {
    try {
        await axios.post(`/api/demo-presenters/${id}?_method=PUT`, { status: newStatus });
...
const deleteEntry = (id: number) => {
    confirm.require({
        message: "Are you sure you want to delete this demo presenter?",
        header: "Confirm Deletion",
        icon: "pi pi-exclamation-triangle",
        acceptClass: "p-button-danger",
        accept: async () => {
            try {
                await axios.delete(`/api/demo-presenters/${id}`);
                toast.add({
                    severity: "success",
                    summary: "Deleted",
                    detail: "Demo Presenter deleted successfully!",
                    life: 3000,
                });
                await fetchEntries();
            } catch (error) {
                toast.add({
                    severity: "error",
                    summary: "Error",
                    detail: "Failed to delete demo presenter.",
                    life: 3000,
                });
            }
        },
...
const submitForm = async () => {
    ...
    try {
        if (editingId.value) {
            await axios.post(`/api/demo-presenters/${editingId.value}?_method=PUT`, payload);
            toast.add({ severity: "success", summary: "Updated", detail: "Demo Presenter updated successfully!", life: 3000 });
            editingId.value = null;
        } else {
            await axios.post("/api/demo-presenters", payload);
            toast.add({ severity: "success", summary: "Created", detail: "Demo Presenter created successfully!", life: 3000 });
        }
        ...
        toast.add({ severity: "error", summary: "Error", detail: "Failed to save demo presenter.", life: 3000 });
    }
};
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <Head title="Demo Presenter Entry" />
        <Toast />
        <ConfirmDialog />
        ...
        <div class="p-6 space-y-8">
            ...
            <Card>
                <template #title>
                    <h2 class="text-xl font-semibold">
                        {{ editingId ? "Edit Demo Presenter" : "Add New Demo Presenter" }}
                    </h2>
                </template>
            ...
            <!-- List Section -->
            <DataTable title="Demo Presenter List" :columns="columns" :rows="tableRows" :onEdit="editEntry"
